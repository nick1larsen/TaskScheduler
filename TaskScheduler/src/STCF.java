...
/**
 * This class simulates the Shortest Time to Completion First (STCF) scheduling. It takes in an array of process information and 
 * creates a new instance of the Process class. It then prioritizes the processes in the STCF method
 * @author Nick
 *
 */
public class STCF extends Algorithm{

	/**
	 * Constructor for the STFC class
	 * @param processArray
	 */
	public STCF(int[][] processArray) {

		super(processArray);	
		System.out.println("Now executing the STCF algorithm");
	}

	/**
	 * This method runs the shortest job first algorithm
	 */
	public void runSTCF() {
		...
		while(stillRunning == true) {
			...
			for(int i = 0; i < super.listOfProcess.length; i++) {
				if(super.listOfProcess[i].getArrivalTime() == super.getTime()) {
					...
					super.STCFAddToActiveProcessQ(super.listOfProcess[i].getProcessID());
